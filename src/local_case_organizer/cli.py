...
def cmd_setup() -> int:
    ...
    payload = {
        ...
        "next_steps": [
            "place private files in data/originals/",
...
def cmd_status() -> int:
    ...
    summary = {
        ...
        "working_file_count": _count_files(paths.working_dir),
        "register_files": _count_files(paths.register_dir),
        "export_packages": len([p for p in paths.exports_dir.iterdir() if p.is_dir()]) if paths.exports_dir.exists() else 0,
    }
